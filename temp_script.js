        ...
        const checkCondition = (cond, executingObj) => {
            const { variable, value, stat } = cond.params || {};
            const operator = cond.params?.operator || '==';
            const obj = executingObj || gameObjects.find(o => o.name === cond.object);
            ...
            switch (cond.trigger) {
                ...
                case 'CompareStat': {
                    ...
                    const statName = cond.params?.stat;
        ...
        const evaluateEvents = (deltaTime = 0) => {
             ...
             currentScene.events.forEach(event => {
                ...
                if (event.conditions.every(checkCondition)) {
                    const isEventTrigger = event.conditions.some(c => ['OnClick', 'OnCollisionWith', 'OnKeyPress', 'OnAttack', 'OnTimerElapsed', 'OnDialogueEnd'].includes(c.trigger));
                    event.actions.forEach(a => executeAction(a, null, !isEventTrigger, deltaTime));
                }
             });
        };
        ...
        function gameLoop(timestamp) {
            ...
            gameObjects.forEach(obj => {
                ...
                if (rpgMovement) {
                    ...
                    // Detect attacks and play animations
                    const isAttackingNow = actionsPressed.attackAction || frameAttacks.includes(obj.name);
                    const nowTime = performance.now();
                    
                    if (isAttackingNow) {
                        const matchingAttackAnim = animations.find(a => {
                    ...
                    if (!isCurrentlyAttacking) {
                        if (obj.vx !== 0 || obj.vy !== 0) {
                            const matchingWalkAnim = animations.find(a => {
                                const nameL = a.name.toLowerCase();
                                const objL = obj.name.toLowerCase();
                            ...
                            const matchingIdleAnim = animations.find(a => {
                                const nameL = a.name.toLowerCase();
                                const objL = obj.name.toLowerCase();
                ...
                if (physics) {
                    ...
                    obj.grounded = false;
                    const gravity = Number(physics.properties?.gravity !== undefined ? physics.properties.gravity : 500);
                    ...
                    for (const solidShape of staticCollisionShapes) {
                        if (obj.id !== solidShape.owner.id && isColliding(getCollisionBox(objWithAbsPosH), solidShape)) {
                            ...
                            obj.vx = 0;
                            absPos = getObjectAbsolutePosition(obj.id, objectsById);
                            objWithAbsPosH = {...obj, ...absPos};
                        }
                    }
                    obj.y += (obj.vy || 0) * deltaTime;
                    absPos = getObjectAbsolutePosition(obj.id, objectsById);
                    let objWithAbsPosV = {...obj, ...absPos};
                    for (const solidShape of staticCollisionShapes) {
                        if (obj.id !== solidShape.owner.id && isColliding(getCollisionBox(objWithAbsPosV), solidShape)) {
                            ...
                            if (objCenterY < solidCenterY) {
                                obj.y = solidShape.y - verticalBox.height - (absPos.y - obj.y);
                            } else {
                                obj.y = solidShape.y + solidShape.height - (absPos.y - obj.y);
                            }
                            obj.vy = 0;
                            absPos = getObjectAbsolutePosition(obj.id, objectsById);
                            objWithAbsPosV = {...obj, ...absPos};
                        }
                    }
                } else {
                    obj.x += (obj.vx || 0) * deltaTime;
            ...
            ctx.imageSmoothingEnabled = window.projectData && window.projectData.hdRendering !== false;
